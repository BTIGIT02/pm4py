...
evaluation_all <- function(eventlog,
                           ...
                           convert = TRUE) {
  pm4py_evaluation <- import("pm4py.algo.evaluation.algorithm", convert = convert)
...
evaluation_precision <- function(eventlog,
                               ...
                               convert = TRUE) {
  pm4py_evaluation <- import("pm4py.algo.evaluation.precision.algorithm", convert = convert)
...
variant_precision_etconformance <- function() {
  pm4py$algo$evaluation$precision$algorithm$ETCONFORMANCE_TOKEN
}
...
evaluation_fitness <- function(eventlog,
                               ...
                               convert = TRUE) {
  pm4py_evaluation <- import("pm4py.algo.evaluation.replay_fitness.algorithm", convert = convert)
...
variant_fitness_token_based <- function() {
  pm4py$algo$evaluation$replay_fitness$algorithm$TOKEN_BASED
}

#' @rdname evaluation
#' @export
variant_fitness_alignment_based <- function() {
  pm4py$algo$evaluation$replay_fitness$algorithm$ALIGNMENT_BASED
}
